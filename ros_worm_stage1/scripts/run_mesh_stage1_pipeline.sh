...
RUN_NAME="${1:-openworm_mesh_$(date +%Y%m%d_%H%M%S)}"
RESULTS_DIR="${ROOT_DIR}/results/${RUN_NAME}"
mkdir -p "${RESULTS_DIR}"

echo "=== Stage-2A ROS Worm pipeline: imported OpenWorm outer-body mesh ==="
echo "Run: ${RUN_NAME}"
...
echo "=== Checking imported STL mesh ==="
python3 analysis/check_stl_mesh.py geometry/openworm/worm_outer_openworm.stl \
  --target-length-mm 1.0 | tee "${RESULTS_DIR}/mesh_check.txt"

echo "=== Transport: X-ray into imported OpenWorm outer-body mesh ==="
./ros_worm macros/run_focused_openworm_mesh.mac | tee "${RESULTS_DIR}/transport.log"
...
echo "=== Chemistry: Geant4-DNA water radiolysis from mesh-transport spectrum ==="
