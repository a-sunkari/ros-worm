...
In aqueous tissue, X-ray interactions generate energetic secondary electrons that ionize and excite surrounding molecules. Furthermore, they can initiate water radiolysis [Might need citation?]. Importantly, LITE-1 function is also sensitive to cellular redox chemistry. LITE-1 and its paralog GUR-3 contribute to behavioral responses to hydrogen peroxide, and the peroxiredoxin PRDX-2 is required for peroxide sensing in several *C. elegans* sensory neurons [5,9]. Reactive oxygen species can also evoke LITE-1-dependent avoidance through the redox modification of Cys44 [7]. Hydrogen peroxide can also suppress some LITE-1-mediated photoresponses [6], suggesting that its effects depend on the cellular and signaling context.
...
[18]. For an effective target concentration $C$, bimolecular rate constant $k$, and competing pseudo-first-order scavenging rate $k_{\mathrm{bg}}$, the fraction of hydroxyl radicals reacting with the target was estimated as

$$
f = \frac{kC}{kC + k_{\mathrm{bg}}}.
$$

Target concentrations from 1 µM to 1 mM and competing background scavenging rates from 10⁸ to 10¹⁰ s⁻¹ were evaluated. H₂O₂/peroxiredoxin reactions were examined separately by integrating the modeled H₂O₂ abundance from 1 ps to 1 µs and applying peroxiredoxin rate constants spanning 10⁵–10⁸ M⁻¹ s⁻¹ [19].
...
Under the concentration and scavenging conditions evaluated, the estimated number of hydroxyl radicals reacting with tryptophan ranged from approximately 6.6 to 5.85 × 10⁵ for the nominal 2 Gy focused nervous system exposure. Corresponding estimates for cysteine ranged from approximately 2.8 to 2.67 × 10⁵ (Fig. 6). Reaction estimates increased with effective target concentration and decreased with increasing background scavenging rate. The modeled H₂O₂ trajectory also overlapped the kinetic range of peroxiredoxin reactions [19]. Because effective target abundance, accessibility, and intracellular scavenging are not known for the irradiated cells, these calculations are meant to define plausible chemical regimes rather than receptor-specific modification yields.
...
Hydroxyl radical chemistry is relevant to the molecular features of LITE-1 implicated in photosensitivity. W77 and W328 are required for the ultraviolet response [4], and free tryptophan reacts rapidly with hydroxyl radicals in aqueous solution [17]. Cysteine reacts at a similarly high rate [18]. Across the effective target concentrations and background scavenging rates examined here, the calculated number of hydroxyl radicals reacting with tryptophan or cysteine varied over several orders of magnitude. However, published rate constants on which this work is based describe free solutes in solution, while residues within the folded receptor will differ in accessibility and local chemical environment. Therefore, these residues must compete with abundant intracellular scavengers and other biomolecular targets. Thus, these calculations offer a preliminary range over which reactions with these residue classes could compete for the initial hydroxyl radical population.

H₂O₂ provides a separate connection between the modeled radiochemistry and known LITE-1 biology. LITE-1 and GUR-3 participate in peroxide sensing, and PRDX-2 is required for H₂O₂ responses in several C. elegans sensory neurons [5,9]. Bischer et al. further showed that ROS can evoke LITE-1-dependent avoidance through redox modification involving Cys44 [7]. Structural and electrophysiological experiments by Hanson et al. identified redox-sensitive elements of LITE-1 and supported a model in which the receptor can respond jointly to photons and H₂O₂ [8]. The relationship is unidirectional, however, because H₂O₂ may suppress some LITE-1-mediated photoresponses [6]. Therefore, production of H₂O₂ by irradiation therefore does not by itself predict an activational LITE-1 response. Redox chemistry could act directly on LITE-1 or through PRDX-2 and other cellular redox pathways, and distinguishing among these possibilities will require further experimentation.
