...
if RUN_BVH_PAIR_SCAN:
    ...
    def get_bvh(obj):
        ...
        # Blender does not provide BVHTree.FromMesh() in many versions.
        # Build a world-space BVH explicitly from evaluated mesh triangles.
    ...
    for k, (_vol, a, b, ex, ey, ez) in enumerate(scan_pairs, 1):
        ...
        try:
            bvh_a = get_bvh(oa)
            bvh_b = get_bvh(ob)
            hits = [] if bvh_a is None or bvh_b is None else bvh_a.overlap(bvh_b)
