...
def export_selected_stl(obj, path):
    """Export selected object using Blender native STL exporters.

    Blender 4.x may expose both wm.stl_export and export_mesh.stl depending
    on the io_mesh_stl add-on. hasattr() is not sufficient by itself: some
    operator proxies appear present but can still fail when called in a given
    context/build. So we try the Blender 4.x operator first, then fall back to
    the legacy add-on operator, and only raise after both fail.
    """
    os.makedirs(os.path.dirname(path), exist_ok=True)

    # Make sure the STL add-on is enabled when available.
    try:
    ...
    try:
        ...
        errors.append(f"wm.stl_export failed: {repr(e)}")

    # Legacy io_mesh_stl add-on operator.
    try:
        ...
        errors.append(f"export_mesh.stl failed: {repr(e)}")

    raise RuntimeError("No working Blender STL export operator. " + " | ".join(errors))
...
def main():
    ...
    print(f"[OpenWorm clean] Blender version: {bpy.app.version_string}")
    print(f"[OpenWorm clean] Blender executable: {bpy.app.binary_path}")
    try:
        bpy.ops.preferences.addon_enable(module="io_mesh_stl")
    except Exception as e:
        print(f"[OpenWorm clean] io_mesh_stl enable attempt: {repr(e)}")
    print(f"[OpenWorm clean] STL operator proxies: wm.stl_import={hasattr(bpy.ops.wm, 'stl_import')} wm.stl_export={hasattr(bpy.ops.wm, 'stl_export')} import_mesh.stl={hasattr(bpy.ops.import_mesh, 'stl')} export_mesh.stl={hasattr(bpy.ops.export_mesh, 'stl')}")
